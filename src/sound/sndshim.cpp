...
// sndplayer change (standalone repo only): the upstream include here is
// "common/util/FileUtil.h", which drags in common's ghc::filesystem chain. The
// local file_io.h provides the same `file_util::read_binary_file` we need, so
// this shim links without `common`. The in-tree copy at game/sound/sndshim.cpp
